...
function DetalhesResiduo() {
  ...
  useEffect(() => {
    ResiduoService.buscarPorCodigo(codigo)
      .then(setResiduo)
      .catch(() => { alert('Resíduo não encontrado.'); navigate('/residuos') })
        ...
        <div className="card-body">
          {residuo.imagem && (
            <figure className="mb-4">
              <img src={residuo.imagem} alt={residuo.nome} className="w-full max-h-64 object-cover rounded-lg" />
            </figure>
          )}
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            ...
            <div>
              ...
              <p className="font-medium">{residuo.quantidade} {residuo.unidadeMedida}</p>
            </div>
          </div>
          {residuo.descricao && (
            <div className="mt-4">
          ...
          <div className="card-actions justify-end mt-6">
            <button onClick={handleDeletar} className="btn btn-error btn-outline btn-sm">Remover</button>
            <Link to={`/residuos/${codigo}/editar`} className="btn btn-primary btn-sm">Editar</Link>
          </div>
        </div>
      </div>
    </div>
